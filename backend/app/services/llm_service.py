...
class LLMService:
    ...
    @staticmethod
    def _next_or_none(
        it: Iterator[CreateChatCompletionStreamResponse],
    ) -> Optional[CreateChatCompletionStreamResponse]:
        """
    ...
    async def get_llm_response_stream(
        self, conversation: List[ChatCompletionRequestMessage]
    ) -> AsyncIterator[str]:
        """
        ...
        try:
            ...
            logger.info(
                f"Generated response with {tokens_generated} chunks in {generation_time:.2f}s"
            )
