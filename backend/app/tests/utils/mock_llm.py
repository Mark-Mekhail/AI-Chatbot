...
class MockLLM:
            ...
            json.dumps(
                {
                    ...
                    "model": "test-model",
                    "choices": [
                        {"index": 0, "delta": {"content": "This is "}, "finish_reason": None}
                    ],
                }
            ),
            json.dumps(
                {
                    ...
                    "model": "test-model",
                    "choices": [
                        {"index": 0, "delta": {"content": "a test "}, "finish_reason": None}
                    ],
                }
            ),
            json.dumps(
    ...
    def setup_mock_llm() -> List[Any]:
        ...
        patches.append(
            patch("app.utils.model_utils.get_model_path", return_value="test_model_path")
        )
        patches.append(
            patch(
                "app.utils.model_utils.get_model_info", return_value=MockLLM.get_mock_model_stats()
            )
        )
